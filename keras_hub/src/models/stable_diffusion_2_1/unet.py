...
class Timesteps(layers.Layer):
    ...
    by the Stable Diffusion v2.1 UNet.
    """
...
class Transformer2DModel(layers.Layer):
    ...
    When `use_linear_projection=True` (the Stable Diffusion v2.x layout), the
    input/output projections are `Dense` layers applied to the flattened
    `(batch, height * width, channels)` sequence. Otherwise they are 1x1
    `Conv2D` layers applied to the spatial feature map (the SD v1.x layout).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_heads = int(num_heads)
        self.head_dim = int(head_dim)
        self.use_linear_projection = bool(use_linear_projection)
        self.inner_dim = self.num_heads * self.head_dim
        ...
        if self.use_linear_projection:
            self.proj_in = layers.Dense(
                self.inner_dim, dtype=self.dtype_policy, name="proj_in"
            )
            self.proj_out = layers.Dense(
                self.inner_dim, dtype=self.dtype_policy, name="proj_out"
            )
        else:
    ...
    def build(self, inputs_shape, context_shape):
        ...
        if self.use_linear_projection:
            ...
            self.proj_in.build((inputs_shape[0], None, channels))
            sequence_shape = (inputs_shape[0], None, self.inner_dim)
            self.transformer_block.build(sequence_shape, context_shape)
            self.proj_out.build((inputs_shape[0], None, self.inner_dim))
        else:
            self.proj_in.build(inputs_shape)
            conv_shape = self.proj_in.compute_output_shape(inputs_shape)
            sequence_shape = (conv_shape[0], None, self.inner_dim)
            self.transformer_block.build(sequence_shape, context_shape)
            self.proj_out.build(conv_shape)
    ...
    def call(self, inputs, context, training=None):
        ...
        width = ops.shape(x)[2]
        if self.use_linear_projection:
            x = ops.reshape(x, (batch_size, height * width, -1))
...
class MidBlock2D(layers.Layer):
    ...
    def __init__(
        self, filters, num_heads, use_linear_projection=False, **kwargs
    ):
        super().__init__(**kwargs)
        self.filters = int(filters)
        self.num_heads = int(num_heads)
...
class UNet(Backbone):
    """The UNet denoising network for Stable Diffusion v2.1.

    Predicts the noise residual added to a latent given the diffusion timestep
    and the encoded text context.

    Compared with the Stable Diffusion v1.x UNet, this network uses a fixed
    attention `head_dim` (so the number of heads scales with the block width),
    a larger `cross_attention_dim` (matching the OpenCLIP text encoder), and
    `Dense` (linear) projections inside the spatial transformers.
    ...
    Args:
        ...
        head_dim: int. The dimension of each attention head. The number of
            heads in each transformer is `block_filters // head_dim`.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def num_heads_for(filters):
            return max(1, int(filters) // head_dim)
