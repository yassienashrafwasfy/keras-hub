...
from keras_hub.src.models.stable_diffusion_2_1.dpm_solver_multistep_scheduler import (  # noqa: E501
    DPMSolverMultistepScheduler,
)
from keras_hub.src.models.stable_diffusion_2_1.unet import UNet
...
@keras_hub_export("keras_hub.models.StableDiffusion2_1Backbone")
class StableDiffusion2_1Backbone(Backbone):
    """Stable Diffusion v2.1 core network with hyperparameters.

    This backbone imports a CLIP model as the text encoder and implements the
    base UNet and VAE networks for the Stable Diffusion v2.1 model. The noise
    schedule is handled by a DPM-Solver++ (2M) scheduler with Karras sigmas.

    Stable Diffusion v2.1 differs from v1.x in its text encoder (an OpenCLIP
    ViT-H/14 model with a larger hidden size), its UNet cross-attention
    dimension and attention head layout, and its `v_prediction` parameterization
    (used by the 768x768 model). The 512x512 base model uses `epsilon`
    prediction.
    ...
    Args:
        ...
        unet_head_dim: int. The dimension of each UNet attention head. The
    ...
    Example:
    ```python
    # Pretrained Stable Diffusion v2.1 model.
    model = keras_hub.models.StableDiffusion2_1Backbone.from_preset(
    ...
    model = keras_hub.models.StableDiffusion2_1Backbone(
