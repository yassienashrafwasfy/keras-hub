...
from keras_hub.src.models.mobileclip2.mobileclip2_layers import MobileCLIP2Head
from keras_hub.src.models.mobileclip2.mobileclip2_layers import (
    MobileCLIP2TextPooler,
)
from keras_hub.src.models.mobileclip2.mobileclip2_layers import (
    MobileCLIP2VisionPooler,
)


@keras_hub_export("keras_hub.models.MobileCLIP2Backbone")
class MobileCLIP2Backbone(Backbone):
    """MobileCLIP2 core network with hyperparameters.

    This backbone implements the base architecture for Apple's MobileCLIP2
    image-text model. It includes a vision and text encoder along with the
    corresponding projection layers. This backbone will output the final logit
    scores corresponding to each image and token input. These values are cosine
    similarities between the corresponding image and text features.

    MobileCLIP2-B pairs a ViT-B/16 vision tower that uses a convolutional patch
    stem (`mci`) with the standard CLIP text transformer.

    The default constructor gives a fully customizable, randomly initialized
    MobileCLIP2 model with any number of layers, heads, and embedding
    dimensions. To load preset architectures and weights, use the `from_preset`
    constructor.

    Args:
        vision_encoder: The MobileCLIP2 vision encoder for encoding the input
            images.
        text_encoder: The MobileCLIP2 text encoder for encoding the input
            tokens.
    ...
    # Pretrained MobileCLIP2 model.
    model = keras_hub.models.MobileCLIP2Backbone.from_preset("mobileclip2_b")
    model(input_data)

    # Randomly initialized MobileCLIP2 model with custom config.
    vision_encoder = keras_hub.models.MobileCLIP2VisionEncoder(
        hidden_dim=768,
        num_layers=12,
        num_heads=12,
    ...
    text_encoder = keras_hub.models.MobileCLIP2TextEncoder(
    ...
    model = keras_hub.models.MobileCLIP2Backbone(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.vision_pooler = MobileCLIP2VisionPooler(
            dtype=dtype, name="vision_pooler"
        )
        self.text_pooler = MobileCLIP2TextPooler(
            dtype=dtype, name="text_pooler"
        )
        self.vision_projection = layers.Dense(
            projection_dim,
            # MobileCLIP-B's image projection is timm's ViT classifier head
            # (`num_classes=projection_dim`), a `Linear` layer that has a bias.
            use_bias=True,
            dtype=dtype,
        ...
        self.mobileclip2_head = MobileCLIP2Head(
            dtype=dtype, name="mobileclip2_head"
        )
        ...
        vision_logits, text_logits = self.mobileclip2_head(
